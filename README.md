...
| `RSI_OVERBOUGHT` / `RSI_OVERSOLD` | Wilder RSI(14) ≥ `rsiOverbought` or ≤ `rsiOversold`. Evaluated on the live, in-progress candle so it fires the instant the threshold is breached (e.g. a 1h spike at 1:01 alerts immediately, not at the 2:00 close). Edge-triggered: dedups by candle so a bar that stays extreme alerts once, with each new still-extreme bar re-firing. Closed bars are still swept so an extreme bar that closed while the monitor was down is not missed. |
